...
There are two big phases in any Temporian script: graph **definition** and **evaluation**. This is a common pattern in computing libraries, and it allows us to perform optimizations before the graph is rund, share Temporian programs across different platforms, and more.
...
Note that you need to pass the [`EventSets`][temporian.EventSet] that correspond to the source [`Nodes`][temporian.Node] in the graph to [`.run()`][temporian.Node.run] (since those are not part of the graph definition). Also, several [`Nodes`][temporian.Node] can be rund at the same time by calling [`tp.run()`][temporian.run] directly.
