...
py::array_t<int> calendar_minute(const py::array_t<double> &timestamps,
  ...
  auto minutes = py::array_t<int>(n_events);

  // Access raw input / output data
  auto v_minutes = minutes.mutable_unchecked<1>();
  ...
  for (Idx i = 0; i < n_events; i++) {
    ...
    // Convert to civil time and get the minute
    auto local_time = absl::ToCivilMinute(utc_time, parsed_tz);
    v_minutes[i] = local_time.minute();
  }
...
void init_calendar_minute(py::module &m) {
