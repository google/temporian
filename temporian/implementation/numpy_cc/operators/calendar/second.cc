...
py::array_t<int> calendar_second(const py::array_t<double> &timestamps,
  ...
  auto seconds = py::array_t<int>(n_events);

  // Access raw input / output data
  auto v_seconds = seconds.mutable_unchecked<1>();
  ...
  for (Idx i = 0; i < n_events; i++) {
    ...
    // Convert to civil time and get the second
    auto local_time = absl::ToCivilSecond(utc_time, parsed_tz);
    v_seconds[i] = local_time.second();
  }
...
void init_calendar_second(py::module &m) {
