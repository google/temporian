...
from temporian.core.operators.calendar.second import CalendarSecondOperator
...
class CalendarSecondNumpyImplementation(OperatorImplementation):
    ...
    def __init__(self, operator: CalendarSecondOperator) -> None:
        ...
        assert isinstance(operator, CalendarSecondOperator)

    def __call__(self, sampling: EventSet) -> Dict[str, EventSet]:
        assert isinstance(self.operator, CalendarSecondOperator)
        ...
        for index_key, index_data in sampling.data.items():
            value = operators_cc.calendar_second(
