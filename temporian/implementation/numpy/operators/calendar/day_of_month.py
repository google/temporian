...
class CalendarDayOfMonthNumpyImplementation(OperatorImplementation):
    ...
    def __init__(self, operator: CalendarDayOfMonthOperator) -> None:
        ...
        assert isinstance(operator, CalendarDayOfMonthOperator)

    def __call__(self, sampling: EventSet) -> Dict[str, EventSet]:
        assert isinstance(self.operator, CalendarDayOfMonthOperator)
        ...
        for index_key, index_data in sampling.data.items():
            value = operators_cc.calendar_day_of_month(
