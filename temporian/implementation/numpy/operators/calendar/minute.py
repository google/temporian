...
from temporian.core.operators.calendar.minute import CalendarMinuteOperator
...
class CalendarMinuteNumpyImplementation(OperatorImplementation):
    ...
    def __init__(self, operator: CalendarMinuteOperator) -> None:
        ...
        assert isinstance(operator, CalendarMinuteOperator)

    def __call__(self, sampling: EventSet) -> Dict[str, EventSet]:
        assert isinstance(self.operator, CalendarMinuteOperator)
        ...
        for index_key, index_data in sampling.data.items():
            value = operators_cc.calendar_minute(
