...
class CalendarDayOfWeekNumpyImplementation(OperatorImplementation):
    ...
    def __init__(self, operator: CalendarDayOfWeekOperator) -> None:
        ...
        assert isinstance(operator, CalendarDayOfWeekOperator)

    def __call__(self, sampling: EventSet) -> Dict[str, EventSet]:
        assert isinstance(self.operator, CalendarDayOfWeekOperator)
        ...
        for index_key, index_data in sampling.data.items():
            value = operators_cc.calendar_day_of_week(
