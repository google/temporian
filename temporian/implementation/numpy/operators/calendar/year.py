...
from temporian.core.operators.calendar.year import CalendarYearOperator
...
class CalendarYearNumpyImplementation(OperatorImplementation):
    ...
    def __init__(self, operator: CalendarYearOperator) -> None:
        ...
        assert isinstance(operator, CalendarYearOperator)

    def __call__(self, sampling: EventSet) -> Dict[str, EventSet]:
        assert isinstance(self.operator, CalendarYearOperator)
        ...
        for index_key, index_data in sampling.data.items():
            value = operators_cc.calendar_year(
