...
from temporian.core.operators.calendar.hour import CalendarHourOperator
...
class CalendarHourNumpyImplementation(OperatorImplementation):
    ...
    def __init__(self, operator: CalendarHourOperator) -> None:
        ...
        assert isinstance(operator, CalendarHourOperator)

    def __call__(self, sampling: EventSet) -> Dict[str, EventSet]:
        assert isinstance(self.operator, CalendarHourOperator)
        ...
        for index_key, index_data in sampling.data.items():
            value = operators_cc.calendar_hour(
